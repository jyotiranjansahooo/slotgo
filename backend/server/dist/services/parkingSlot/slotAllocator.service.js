...
class SlotAllocatorService {
    ...
    async reserveAvailableSlot(parkingId, vehicleType) {
        ...
        if (!slot) {
            throw new ApiError(409, "No parking capacity is available for this vehicle type.");
        }
        return slot;
    }
    ...
    async occupySlot(slotId) {
        ...
        if (!slot) {
            throw new ApiError(404, "Parking slot not found or no capacity is available.");
        }
        return slot;
    }
}
