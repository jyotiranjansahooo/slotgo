...
import { generateOtp, hashOtp, getOtpExpiry, getMaxOtpAttempts, verifyOtp, } from "../../utils/otp.js";
...
|--------------------------------------------------------------------------
*/
export const requestParkingActionVerification = async ({ ownerId, parkingId, action, }) => {
    /*
    ...
    if (!parking) {
        throw new ApiError(404, "Parking not found or you are not authorized to modify it.");
    }
    /*
     ...
     * Find the owner.
     */
    const owner = await User.findById(ownerId).select("+actionVerificationOtpHash " +
     ...
     */
    if (!owner.isVerified) {
        throw new ApiError(403, "Please verify your email before performing this action.");
    }
    /*
     * Prevent OTP spam.
     */
...
|--------------------------------------------------------------------------
*/
export const verifyParkingActionVerification = async ({ ownerId, action, otp, }) => {
    const owner = await User.findById(ownerId).select("+actionVerificationOtpHash " +
        "+actionVerificationOtpExpiresAt " +
        "+actionVerificationOtpAttempts " +
        "+actionVerificationType");
    ...
    if (owner.actionVerificationType !== action) {
        throw new ApiError(400, "This verification code is not valid for this action. Please request a new code.");
    }
    /*
     * Check expiry.
     */
...
|--------------------------------------------------------------------------
*/
function clearActionVerificationOtp(owner) {
    owner.actionVerificationOtpHash = "";
    owner.actionVerificationOtpExpiresAt = undefined;
    owner.actionVerificationOtpAttempts = 0;
    owner.actionVerificationType = undefined;
}
//# sourceMappingURL=parkingActionVerification.service.js.map