...
class AdminService {
    ...
    async updateUserRole(targetUserId, newRole, currentAdminId) {
        ...
        if (targetUser._id.toString() === currentAdminId) {
            throw new ApiError(400, "You cannot change your own administrator role.");
        }
        /*
        ...
        if (targetUser.role === newRole) {
            throw new ApiError(400, `User is already a ${newRole}.`);
        }
    ...
    async getDashboardStats() {
        ...
        const approvedParkings = parkings.filter((parking) => parking.status === "approved" &&
            parking.isActive).length;
        const pendingParkings = parkings.filter((parking) => parking.status === "pending").length;
        const rejectedParkings = parkings.filter((parking) => parking.status === "rejected").length;
        // ----------------------------------------------------------
        // BOOKING STATISTICS
        // ----------------------------------------------------------
        ...
        const confirmedBookings = bookings.filter((booking) => booking.bookingStatus ===
            "confirmed").length;
        const activeBookings = bookings.filter((booking) => booking.bookingStatus ===
            "active").length;
        const completedBookings = bookings.filter((booking) => booking.bookingStatus ===
            "completed").length;
        const cancelledBookings = bookings.filter((booking) => booking.bookingStatus ===
            "cancelled").length;
        const expiredBookings = bookings.filter((booking) => booking.bookingStatus ===
            "expired").length;
        // ----------------------------------------------------------
        // PAYMENT STATISTICS
        // ----------------------------------------------------------
        ...
        const totalRevenue = successfulPayments.reduce((total, payment) => total + payment.amount, 0);
        const totalRefunded = payments.reduce((total, payment) => total + payment.refundAmount, 0);
        return {
            users: {
                total: totalUsers,
                active: activeUsers,
