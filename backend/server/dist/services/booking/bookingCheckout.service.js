...
import { BOOKING_MODE_VALUES, BOOKING_STATUS, PAYMENT_STATUS as BOOKING_PAYMENT_STATUS, } from "../../constants/booking.js";
...
class BookingCheckoutService {
    ...
    async createCheckout(driverId, data) {
        ...
        if (!BOOKING_MODE_VALUES.includes(data.bookingMode)) {
            throw new ApiError(400, "Invalid booking mode.");
        }
        ...
        let razorpayOrder;
        try {
            razorpayOrder = await razorpayService.createOrder(Math.round(pricing.driverPays * 100), `checkout-${Date.now()}`);
        }
        catch (error) {
            await slotAllocatorService.releaseSlot(slot._id.toString());
            throw error;
    ...
    async getCheckout(driverId, checkoutId) {
        ...
        if (checkout.expiresAt <= new Date()) {
            ...
            throw new ApiError(410, "This checkout has expired. Please start the booking again.");
        }
        return checkout;
    }
    async verifyCheckoutPayment(driverId, checkoutId, orderId, paymentId, signature) {
        if (!Types.ObjectId.isValid(checkoutId)) {
            throw new ApiError(400, "Invalid checkout ID.");
        }
        if (!orderId?.trim()) {
        ...
        const checkout = await bookingCheckoutRepository.findById(checkoutId);
        if (!checkout) {
            throw new ApiError(404, "Booking checkout not found or already completed.");
        }
        if (checkout.driverId.toString() !== driverId) {
            throw new ApiError(403, "You are not authorized to complete this checkout.");
        }
        ...
        if (!vehicle) {
            throw new ApiError(404, "Vehicle associated with checkout was not found.");
        }
        if (vehicle.ownerId.toString() !== driverId) {
        ...
        if (!parking) {
            throw new ApiError(404, "Parking associated with checkout was not found.");
        }
        if (parking.status !== PARKING_STATUS.APPROVED) {
        ...
        const slot = await slotAllocatorService.confirmReservation(checkout.slotId.toString());
        if (!slot) {
            throw new ApiError(409, "Parking capacity reservation could not be confirmed.");
        }
        ...
        let walletResult;
        try {
            walletResult = await walletService.creditOwnerEarnings(booking.ownerId.toString(), booking.ownerReceives, booking._id.toString(), `booking:${booking._id.toString()}`, `Earnings from booking ${booking.bookingNumber}`);
        }
        catch (error) {
            return {
                payment,
                ...
                wallet: null,
                transaction: null,
                walletError: "Booking and payment were completed, but owner wallet credit could not be completed automatically.",
            };
        }
        return {
            payment,
...
//# sourceMappingURL=bookingCheckout.service.js.map