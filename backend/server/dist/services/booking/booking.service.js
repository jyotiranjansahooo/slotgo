...
class BookingService {
        ...
        switch (bookingMode) {
            ...
            case "hourly": {
                ...
                if (!Number.isInteger(durationHours)) {
                    throw new ApiError(400, "Hourly booking duration must be a whole number of hours.");
                }
                break;
            }
            // --------------------------------------------------------
            ...
            case "daily": {
                ...
                if (durationHours % 24 !== 0) {
                    throw new ApiError(400, "Daily booking duration must be in complete days.");
                }
                break;
            }
            // --------------------------------------------------------
    ...
    calculateCancellationRefund(booking) {
        const now = new Date();
        const remainingMs = booking.startTime.getTime() -
            now.getTime();
        ...
        const refundAmount = Number((booking.driverPays *
            (refundPercentage / 100)).toFixed(2));
        const penaltyAmount = Number((booking.driverPays -
    ...
    async createBooking(driverId, data) {
        ...
        try {
            pricing =
                pricingService.calculate(parking, vehicle.vehicleType, data.bookingMode, data.startTime, data.endTime);
        }
        catch (error) {
            await slotAllocatorService.releaseSlot(slot._id.toString());
            throw error;
        }
        if (pricing.parkingAmount <=
            0) {
            await slotAllocatorService.releaseSlot(slot._id.toString());
    ...
    async getBooking(userId, bookingId) {
        ...
        const isDriver = booking.driverId.toString() ===
            userId;
        const isOwner = booking.ownerId.toString() ===
            userId;
        if (!isDriver && !isOwner) {
    ...
    async cancelBooking(driverId, bookingId, data) {
        ...
        if (booking.paymentStatus ===
            ...
            if (payment.status ===
                ...
                if (refundAmount > 0) {
                    refundedPayment =
                        await paymentService.refundPayment(payment._id.toString(), refundAmount);
                }
            }
        }
        // ----------------------------------------------------------
        ...
        return {
            ...
            wallet: refundedPayment?.wallet ??
                null,
            transaction: refundedPayment?.transaction ??
                null,
        };
    }
    // ============================================================
