...
class WalletService {
    ...
    async creditOwnerEarnings(ownerId, amount, bookingId, referenceId, description = "Parking booking earnings") {
        ...
        // UPDATE WALLET
        // ----------------------------------------------------------
        const updatedWallet = await walletRepository.updateBalance(wallet._id.toString(), {
    ...
    async reverseOwnerEarnings(ownerId, amount, bookingId, referenceId, description = "Booking refund") {
        ...
        if (wallet.availableBalance < amount) {
            throw new ApiError(400, "Owner wallet balance is insufficient for refund reversal.");
        }
        // ----------------------------------------------------------
        // CALCULATE BALANCE
        // ----------------------------------------------------------
    ...
    async withdraw(ownerId, amount, referenceId, description = "Wallet withdrawal") {
        if (amount <= 0) {
