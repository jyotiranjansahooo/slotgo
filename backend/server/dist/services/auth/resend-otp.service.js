...
export const resendOtpService = async (data) => {
    ...
    const user = await User.findOne({
        email,
    }).select("+verificationOtpHash +verificationOtpExpiresAt +verificationOtpAttempts");
    if (!user) {
    ...
    if (user.verificationOtpExpiresAt &&
        ...
        if (elapsed < RESEND_COOLDOWN_MS) {
            ...
            throw new ApiError(429, `Please wait ${remainingSeconds} seconds before requesting another code.`);
        }
    }
