...
export const getVehicle = asyncHandler(async (req, res) => {
    const vehicleId = Array.isArray(req.params.id)
        ? req.params.id[0]
        : req.params.id;
    const vehicle = await vehicleService.getById(req.user._id.toString(), vehicleId);
    res
        .status(200)
...
export const updateVehicle = asyncHandler(async (req, res) => {
    const vehicleId = Array.isArray(req.params.id)
        ? req.params.id[0]
        : req.params.id;
    const vehicle = await vehicleService.update(req.user._id.toString(), vehicleId, req.body);
    res
        .status(200)
...
export const setDefaultVehicle = asyncHandler(async (req, res) => {
    const vehicleId = Array.isArray(req.params.id)
        ? req.params.id[0]
        : req.params.id;
    const vehicle = await vehicleService.setDefault(req.user._id.toString(), vehicleId);
    res
        .status(200)
        .json(new ApiResponse(200, "Default vehicle updated", vehicle));
});
