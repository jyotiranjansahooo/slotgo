...
export const updateUserRole = asyncHandler(async (req, res) => {
    ...
    const user = await adminService.updateUserRole(userId, role, currentAdminId);
    return res
        .status(200)
...
export const getParkingBookings = asyncHandler(async (req, res) => {
    const parkingId = req.params.parkingId;
    const bookings = await adminService.getParkingBookings(parkingId);
    return res
        .status(200)
        .json(new ApiResponse(200, bookings, "Parking bookings fetched successfully."));
});
// ADMIN DASHBOARD
