...
export const getAvailableSlots = asyncHandler(async (req, res) => {
    ...
    if (typeof vehicleType !== "string") {
        return res
            .status(400)
            .json(new ApiResponse(400, null, "vehicleType query parameter is required."));
    }
    const result = await parkingDiscoveryService.getAvailableSlots(id, vehicleType);
    return res
        .status(200)
        .json(new ApiResponse(200, result, "Available parking slots fetched successfully."));
});
...
export const searchParkings = asyncHandler(async (req, res) => {
    ...
    const parkings = await parkingDiscoveryService.searchParkings({
        ...
        parkingType: typeof parkingType === "string" ? parkingType : undefined,
    });
    return res
        .status(200)
        .json(new ApiResponse(200, parkings, "Parkings fetched successfully."));
...
export const getParkingDetails = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, result, "Parking details fetched successfully."));
});
