...
export const getAvailableSlots = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const result = await parkingDiscoveryService.getAvailableSlots(
      id as string,
...
export const getParkingDetails = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const result = await parkingDiscoveryService.getParkingDetails(
      id as string,
    );
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, result, "Parking details fetched successfully."),
      );
  },
);
