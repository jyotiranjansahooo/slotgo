...
export const approveParking = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const parking = await parkingRepository.findById(id as string);

    if (!parking) {
      throw new ApiError(404, "Parking not found.");
    }

    const updatedParking = await parkingRepository.approve(id as string);
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, updatedParking, "Parking approved successfully."),
      );
  },
);
...
export const rejectParking = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const updatedParking = await parkingRepository.reject(id as string);
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, updatedParking, "Parking rejected successfully."),
      );
  },
);
