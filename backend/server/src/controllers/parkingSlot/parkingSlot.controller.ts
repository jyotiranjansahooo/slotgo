...
export const getParkingSlots = asyncHandler(
  async (req: Request, res: Response) => {
    const parkingId = getParamString(
...
export const createParkingSlot = asyncHandler(
  async (req: Request, res: Response) => {
    const parkingId = getParamString(
      req.params.parkingId,
      "Invalid parking ID.",
    ...
    if (existingSlot) {
      throw new ApiError(
        409,
        `Slot ${req.body.slotNumber} already exists in this parking.`,
      );
    }
...
export const updateParkingSlot = asyncHandler(
  async (req: Request, res: Response) => {
    const parkingId = getParamString(
      req.params.parkingId,
      "Invalid parking ID.",
    ...
    if (slot.parkingId.toString() !== parkingId) {
      throw new ApiError(
        403,
        "This parking slot does not belong to this parking.",
      );
    }
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, updatedSlot, "Parking slot updated successfully."),
      );
  },
);
...
export const deleteParkingSlot = asyncHandler(
  async (req: Request, res: Response) => {
    const parkingId = getParamString(
      req.params.parkingId,
      "Invalid parking ID.",
