...
import {
  CreateParkingBookingBlockInput,
...
class ParkingBookingBlockService {
  async create(
    ...
    if (data.startTime < new Date()) {
      throw new ApiError(
        400,
        "Booking pause cannot start in the past.",
      );
    }

    const overlappingBlock =
      await parkingBookingBlockRepository.findOverlapping(
        parkingId,
        data.startTime,
        data.endTime,
      );
    ...
    if (overlappingBlock) {
      throw new ApiError(
        409,
        "This parking already has a booking pause during the selected time.",
      );
    }
  ...
  async getMyBlocks(
    ...
    return parkingBookingBlockRepository.findByParking(
      parkingId,
    );
  }
  ...
  async delete(
    ...
    const block =
      await parkingBookingBlockRepository.findById(
        blockId,
      );
    ...
    if (!block) {
      throw new ApiError(
        404,
        "Booking pause not found.",
      );
    }

    const parking =
      await parkingRepository.findById(
        block.parkingId.toString(),
      );
    ...
    return parkingBookingBlockRepository.delete(
      blockId,
    );
  }
}
